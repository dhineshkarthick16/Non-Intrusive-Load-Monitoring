"""
collect_7min_dataset.py
-----------------------
7-Minute Automated 4-Class Hardware Dataset Collection Protocol.

Protocol Schedule (7 cycles x 1 minute = 7 minutes total):
  Cycle 1 (00:00 - 01:00): Expected -> 'NO LOAD' (Quiescent baseline noise)
  Cycle 2 (01:00 - 02:00): Expected -> 'Charger' (Phone Charger ON)
  Cycle 3 (02:00 - 03:00): Expected -> 'NO LOAD' (Charger OFF & Removed)
  Cycle 4 (03:00 - 04:00): Expected -> 'Bulb' (Incandescent Bulb ON)
  Cycle 5 (04:00 - 05:00): Expected -> 'NO LOAD' (Bulb OFF)
  Cycle 6 (05:00 - 06:00): Expected -> 'Both' (Both Charger + Bulb ON simultaneously)
  Cycle 7 (06:00 - 07:00): Expected -> 'NO LOAD' (All appliances OFF & Removed)

Features:
  - 5-second transition alerts with audible countdown beeps at every phase change.
  - Windows Media speaker audio chimes (ding.wav, tada.wav, chimes.wav) + motherboard beeps.
  - Continuous streaming from COM3 @ 115200 baud.
  - Flushes raw (V1, V2, V3) and ground-truth labels directly to 'dataset_7min_4class.csv'.
  - Writes live progress to 'collection_progress_7min.json'.
"""
...
try:
    ...
    def beep_phase_complete():
        """Loud 'chimes' sound notifying that the 1-minute phase has completed."""
        play_wav_or_beep("chimes.wav", fallback_freq=1500, fallback_ms=350)

    def beep_test_all_done():
        """Final victory chime when the entire 7-minute test finishes."""
        play_wav_or_beep("tada.wav", fallback_freq=2000, fallback_ms=600)
...
PROTOCOL_CYCLES_7MIN = [
    {
        "cycle": 1,
        "name": "Phase 1 (0:00 - 1:00)",
        "expected": "NO LOAD",
...
def update_progress_file(progress_path: str, data: dict):
    """Write live progress JSON for external monitoring."""
    try:
...
def countdown_transition_alert(cycle_info: dict, countdown_sec: int = 5):
    ...
    beep_transition_go()
    print("\r  [+] PHASE STARTED! Recording data for 60 seconds (1 minute)...          \n")


def collect_dataset(port="COM3", baud=115200, output_csv="dataset_7min_4class.csv", cycle_duration=60):
    """
    Main collection loop running across the 7 protocol cycles (1 min each).
    """
    abs_csv = os.path.abspath(output_csv)
    prog_file = os.path.abspath("collection_progress_7min.json")

    print("=" * 80)
    print("NILM 7-MINUTE 4-CLASS DATASET COLLECTION PROTOCOL (1 MINUTE PER PHASE)")
    print("=" * 80)
    ...
    try:
        for cycle_info in PROTOCOL_CYCLES_7MIN:
            expected_label = cycle_info["expected"]
            cycle_idx = cycle_info["cycle"]
            duration = cycle_duration

            # Alert and 5-sec countdown beeps at the transition
            ...
            while (time.time() - cycle_start) < duration:
                ...
                update_progress_file(prog_file, {
                    "cycle": cycle_idx,
                    "phase_name": cycle_info["name"],
                    "expected_label": expected_label,
                    "cycle_elapsed": int(cycle_elapsed),
                    "cycle_duration": duration,
                    "seconds_remaining": rem_sec,
                    "samples_in_cycle": samples_in_cycle,
    ...
    # Summary table
    print("\n" + "=" * 65)
    print("7-MINUTE 4-CLASS COLLECTION SUMMARY")
    print("=" * 65)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="7-Minute NILM 4-Class Dataset Collection Protocol")
    parser.add_argument("--port", type=str, default="COM3", help="Serial port (default: COM3)")
    parser.add_argument("--baud", type=int, default=115200, help="Baud rate (default: 115200)")
    parser.add_argument("--output", type=str, default="dataset_7min_4class.csv", help="Output CSV path")
    parser.add_argument("--duration", type=int, default=60, help="Duration per cycle in seconds (default: 60)")
